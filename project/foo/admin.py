...
class ThingyView(ProtectedModelView):
    column_list = ('image',)
    form_excluded_columns = ('image_storage_type', 'image_storage_bucket_name')
    ...
    def scaffold_form(self):
        ...
        static_root_parent = path.abspath(path.join(app.config['PROJECT_ROOT'], '..'))

        if app.config['USE_S3']:
            form_class.image.kwargs['storage_type'] = 's3'

        form_class.image.kwargs['bucket_name'] = app.config['S3_BUCKET_NAME']
        form_class.image.kwargs['access_key_id'] = app.config['AWS_ACCESS_KEY_ID']
        form_class.image.kwargs['access_key_secret'] = app.config['AWS_SECRET_ACCESS_KEY']
